"""JM-Epi-only P5 eligibility, explicit DeepCDR routes and deterministic Top-k.
...
def select_records(records, policy, *, excluded_pairs=()):
    ...
    for rank, row in enumerate(selected, 1):
        row.update(selection_rank=rank, selection_primary_score=-rank_key(row)[0],
                   common_deepcdr_floor=policy["common_deepcdr_floor"])
        decisions.append({"id": row["id"], "state": "selected", "rank": rank})
    for row in unique[policy["target"]:]:
        decisions.append({"id": row["id"], "state": "eligible_not_top_k"})
    counts.update(eligible_before_dedup=len(eligible), eligible_unique=len(unique),
                  selected=len(selected), eligible_not_top_k=max(0, len(unique) - len(selected)))
    boundary = None
    if selected:
        last = selected[-1]
    ...
    return selected, {"status": "complete" if len(selected) == policy["target"] else "insufficient_eligible",
        "counts": dict(counts), "policy": policy, "actual_boundary": boundary,
        "decisions": sorted(decisions, key=lambda d: order[d["id"]]),
        "ranking": "active scores descending (max, min for OR), likelihood descending, id ascending",
